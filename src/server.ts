...
} from "./telemetry/fixed-instrumentation";
import { testMetricRecording, getMetricsStatus, shutdownMetrics } from "./telemetry/simple-metrics";
...
async function handleTokenRequest(req: Request, span: any): Promise<Response> {
  ...
  try {
    ...
    telemetryLogger.logAuthenticationEvent('header_validation', true, {
      userId: username,
      consumerId,
      requestId,
    });
    ...
    if (!consumerSecret) {
      ...
      telemetryLogger.logKongOperation('create_consumer_secret', 0, true, {
        consumerId,
      });
    ...
    telemetryLogger.logAuthenticationEvent('token_issued', true, {
      userId: username,
      requestId,
      duration,
    });
...
async function handleHealthCheck(span: any): Promise<Response> {
  ...
  try {
    ...
    telemetryLogger.logHttpRequest('GET', '/health', status, duration, {
...
function handleTelemetryHealth(): Response {
  ...
  return tracer.startActiveSpan("GET /health/telemetry", {
    ...
    try {
      ...
      telemetryLogger.logHttpRequest('GET', '/health/telemetry', 200, duration, {
        telemetry_initialized: telemetryStatus.initialized
      });

      const responseSize = JSON.stringify(telemetryStatus, null, 2).length;
      recordHttpRequest('GET', '/health/telemetry', 200, undefined, responseSize);
    ...
    const metricsHealth = {
      ...
      metrics: {
        ...
        available_metrics: metricsStatus.availableMetrics || {
          http: ['requests_total', 'response_time_seconds', 'requests_by_status_total', 'active_connections', 'request_size_bytes', 'response_size_bytes'],
          system: ['memory_usage_bytes', 'heap_usage_bytes', 'cpu_usage_percent', 'uptime_seconds', 'active_handles'],
          business: ['jwt_tokens_issued_total', 'jwt_token_creation_duration_seconds', 'authentication_attempts_total', 'authentication_success_total', 'authentication_failures_total', 'kong_operations_total', 'kong_operation_duration_seconds', 'kong_cache_hits_total', 'kong_cache_misses_total'],
          errors: ['application_errors_total', 'application_exceptions_total', 'telemetry_exports_total', 'telemetry_export_errors_total', 'circuit_breaker_state', 'operation_duration_seconds']
        },
        instruments: {
...
function handleDebugMetricsTest(): Response {
  ...
  try {
    ...
    recordHttpRequest('POST', '/debug/metrics/test', 200, undefined, responseSize);
...
async function handleDebugMetricsFlush(): Promise<Response> {
  ...
  try {
    ...
    const result = {
      ...
      export_attempts_since_flush: statsAfter.totalExports - statsBefore.totalExports,
      endpoint: getTelemetryStatus().config.METRICS_ENDPOINT
    };
    
    const duration = (Bun.nanoseconds() - startTime) / 1_000_000;
    const responseSize = JSON.stringify(result, null, 2).length;
    recordHttpRequest('POST', '/debug/metrics/flush', 200, undefined, responseSize);
...
function handleDebugMetricsStats(): Response {
  ...
  try {
    ...
    const duration = (Bun.nanoseconds() - startTime) / 1_000_000;
    const responseSize = JSON.stringify(result, null, 2).length;
    recordHttpRequest('GET', '/debug/metrics/stats', 200, undefined, responseSize);
...
function handleMetrics(): Response {
  ...
  return tracer.startActiveSpan("GET /metrics", {
    kind: 1, // SpanKind.SERVER
    attributes: {
      "http.request.method": "GET",
      "http.route": "/metrics",
    ...
    try {
      ...
      telemetryLogger.logHttpRequest('GET', '/metrics', 200, duration, {
        uptime: Math.floor(process.uptime())
      });
...
log("Checking Kong connectivity...", { component: 'kong', event: 'connectivity_check' });

const startupKongHealth = await kongService.healthCheck();
// Record startup Kong health check
recordKongOperation('startup_health_check', startupKongHealth.responseTime, startupKongHealth.healthy);
...
if (!startupKongHealth.healthy) {
  ...
  telemetryLogger.logKongOperation('health_check', startupKongHealth.responseTime, false, {
    error: startupKongHealth.error,
  });
  ...
  telemetryLogger.logKongOperation('health_check', startupKongHealth.responseTime, true, {
    operation: 'health_check'
  });
}
...
try {
  ...
  const tracer = trace.getTracer('pvh-authentication-service', '1.0.0');
      ...
      return tracer.startActiveSpan(`${req.method} ${url.pathname}`, {
        ...
        try {
          if (url.pathname === '/health') {
            ...
            telemetryLogger.logHttpRequest(req.method, url.pathname, 404, duration, {
              requestId,
            });
            ...
            recordHttpRequest(req.method, url.pathname, 404, undefined, notFoundResponse.length);
  ...
  if (err instanceof Error && err.message.includes('EADDRINUSE')) {
    ...
    log("To stop existing servers, run: pkill -f 'bun src/server.ts' or lsof -ti:3000 | xargs kill", {
      component: 'server',
...
log("Metrics debugging endpoints available", {
  ...
  usage: {
    test_and_check: 'curl -X POST http://localhost:3000/debug/metrics/test && sleep 15 && curl http://localhost:3000/debug/metrics/stats',
    manual_flush: 'curl -X POST http://localhost:3000/debug/metrics/flush',
    view_stats: 'curl http://localhost:3000/debug/metrics/stats'
  }
});

log("Telemetry configuration loaded", {
  component: 'telemetry',
  event: 'configuration',
  success: true,
  'telemetry.service': getSimpleTelemetryStatus().config.SERVICE_NAME,
  'telemetry.environment': getSimpleTelemetryStatus().config.DEPLOYMENT_ENVIRONMENT,
  'telemetry.mode': getSimpleTelemetryStatus().config.TELEMETRY_MODE,
  'telemetry.traces_endpoint': getSimpleTelemetryStatus().config.TRACES_ENDPOINT,
  'telemetry.logs_endpoint': getSimpleTelemetryStatus().config.LOGS_ENDPOINT,
  'telemetry.metrics_endpoint': getSimpleTelemetryStatus().config.METRICS_ENDPOINT,
});
