...
The `reinitialize()` method exists because PinoInstrumentation monkey-patches `pino()` at registration time. If the logger is created before the OTEL SDK starts, it will not have the OTelPinoStream attached. Calling `reinitialize()` after SDK startup recreates the Pino instance with instrumentation active.
...
- OTLP log export via PinoInstrumentation's OTelPinoStream
...
Every log line should include the OpenTelemetry trace and span IDs so that logs can be correlated with distributed traces in your observability platform.

### Pino Mixin

A Pino mixin runs on every log call and merges its return value into the log record.
...
PinoInstrumentation has a built-in log correlation feature (`disableLogCorrelation: false`) that injects trace context into log records. However, it uses **underscore notation**:

| PinoInstrumentation built-in | ECS dot-notation (what we use) |
|------------------------------|-------------------------------|
| `trace_id` | `trace.id` |
| `span_id` | `span.id` |
| `trace_flags` | (not needed) |

Elasticsearch and Kibana expect ECS dot-notation fields. If you use the built-in correlation, your logs will have `trace_id` instead of `trace.id`, and trace-to-log correlation in Kibana will not work.

**Solution**: Disable PinoInstrumentation's built-in correlation (`disableLogCorrelation: true`) and use the custom mixin above. This gives you ECS-compliant field names while still benefiting from PinoInstrumentation's OTLP log export (OTelPinoStream).
...
PinoInstrumentation from `@opentelemetry/instrumentation-pino` enables automatic log export to your OTLP collector alongside traces and metrics.

### How It Works

PinoInstrumentation monkey-patches the `pino()` constructor. When your code calls `pino(options, destination)`, the instrumentation wraps it and adds an `OTelPinoStream` via `pino.multistream()`.

```
pino.info("message", context)
    |
    v
Original destination (stdout)     -- your console/file output
    +
...
BatchLogRecordProcessor
    |
    v
OTLPLogExporter  -->  OTLP Collector (Elasticsearch, Grafana, etc.)
```

This means every log call produces output in **two places**: your configured destination (stdout) and the OTLP pipeline. No code changes required beyond registering the instrumentation.
...
const pinoInstrumentation = new PinoInstrumentation({
  ...
  // Disable built-in log correlation -- we use our own ECS mixin
  // (built-in uses underscore format: trace_id, span_id)
  disableLogCorrelation: true,

  // Keep OTelPinoStream active for OTLP export
...
| Option | Value | Reason |
|--------|-------|--------|
| `enabled` | `true` | Activate the instrumentation |
| `disableLogCorrelation` | `true` | We inject trace context ourselves via the ECS mixin (dot-notation) |
| `disableLogSending` | `false` | Enables OTelPinoStream for OTLP log export |

### Registration Order

PinoInstrumentation must be registered **before** the first call to `pino()`. If your logger is created at module load time, register the instrumentation in your OTEL setup file that runs before the application entry point.

```typescript
// src/telemetry/setup.ts -- runs before src/index.ts
...
### Reinitialize After SDK Start

If your logger was created before the OTEL SDK started (common in module-level singletons), the Pino instance will not have the OTelPinoStream. Call `reinitialize()` after SDK startup to recreate the logger with instrumentation active.

```typescript
// After sdk.start() completes:
...
- **Check LoggerProvider is registered**: The OTEL SDK must have a `LoggerProvider` with a `BatchLogRecordProcessor` and `OTLPLogExporter` configured.
- **Check PinoInstrumentation is registered**: It must be added to the SDK's `instrumentations` array before the first `pino()` call.
- **Check `disableLogSending` is `false`**: If set to `true`, OTelPinoStream is not created and no logs are exported.
- **Reinitialize after SDK start**: If the logger was created before the SDK started, call `reinitialize()` to recreate the Pino instance with the OTelPinoStream attached.
- **Check collector connectivity**: Verify the OTLP endpoint is reachable with `curl -X POST http://localhost:4318/v1/logs`.
...
- **PinoInstrumentation adds a second stream**: This is expected. One line goes to your destination (stdout), the other goes to OTelPinoStream (OTLP). They should not both appear on stdout.
- **If both appear on stdout**: Check that `pino.multistream()` is not being called manually in addition to PinoInstrumentation.
...
6. **Reinitialize after OTEL SDK start** -- ensures the Pino instance has OTelPinoStream attached for OTLP export.
