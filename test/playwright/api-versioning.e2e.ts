...
test.describe('API Versioning - V1 vs V2 Behavior', () => {
  ...
  test.describe('Health Endpoint Version Differences', () => {
    ...
    test('default health endpoint behavior uses v1 for backward compatibility', async ({ request }) => {
      const response = await request.get('/health');
  ...
  test.describe('Token Endpoint Version Differences', () => {
    ...
    test('v2 token endpoint provides enhanced security headers and audit logging', async ({ request }) => {
      const response = await request.get('/tokens', {
        headers: {
          'Accept-Version': 'v2',
          'X-Consumer-Id': TEST_CONSUMER.id,
    ...
    test('default token endpoint behavior uses v1 for backward compatibility', async ({ request }) => {
      const response = await request.get('/tokens', {
        headers: {
          'X-Consumer-Id': TEST_CONSUMER.id,
          'X-Consumer-Username': TEST_CONSUMER.username
    ...
    test('v1 and v2 generate functionally identical JWT tokens', async ({ request }) => {
      ...
      // Parse JWT payloads
      const v1Payload = JSON.parse(Buffer.from(v1Data.access_token.split('.')[1], 'base64url').toString());
      const v2Payload = JSON.parse(Buffer.from(v2Data.access_token.split('.')[1], 'base64url').toString());
  ...
  test.describe('Version Header Validation', () => {
    ...
    test('should use v1 for backward compatibility when no Accept-Version header provided', async ({ request }) => {
      const response = await request.get('/health');
  ...
  test.describe('OpenAPI Specification Version Support', () => {
    test('OpenAPI spec should document both v1 and v2 endpoints', async ({ request }) => {
      ...
      if (spec.components?.parameters) {
        const versionParam = Object.values(spec.components.parameters).find((param: any) =>
          param.name === 'Accept-Version'
        );
        if (versionParam) {
